    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute z-20 top-full mt-1 left-0 right-0 bg-stone-800 border border-stone-700 rounded shadow-xl overflow-hidden">
            {COLOR_THEMES.map(t => (
              <button
                key={t.value}
          ...
          <div className="mt-3">
            ...
            {settings.transition === 'crossfade' && settings.transition_duration > settings.max_clip_duration / 2 && (
              <p className="text-[10px] text-amber-500/80 mt-1">
                Crossfade ({settings.transition_duration}s) is at least half the max clip duration ({settings.max_clip_duration}s). Reduce transition or increase clip duration.
              </p>
            )}
          </div>
        )}
