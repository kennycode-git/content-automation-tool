...
def grade_mocha(img: Image.Image, intensity: float = 0.92) -> Image.Image:
    """Dark academia: deep shadows with rich coffee/caramel warmth.
    Darkens overall, crushes blue hard, lifts red+green for amber cast,
    then high contrast so shadows go near-black while warm tones glow.
    """
    base = img.convert("RGB")
    arr = np.array(base, dtype=np.float32)
    # Darken overall — dark but not abyss-level
    arr = (arr * 0.62).clip(0, 255)
    # Coffee/caramel cast: crush blue, lift red+green (red+green = amber)
    arr[:, :, 0] = (arr[:, :, 0] * 1.28).clip(0, 255)  # lift red
    arr[:, :, 1] = (arr[:, :, 1] * 1.10).clip(0, 255)  # lift green slightly → amber
    arr[:, :, 2] = (arr[:, :, 2] * 0.55).clip(0, 255)  # crush blue hard
    graded = Image.fromarray(arr.astype(np.uint8), "RGB")
    # High contrast: shadows go very dark, warm midtones pop
    graded = ImageEnhance.Contrast(graded).enhance(1.55)
    # Saturation boost so the caramel feels rich not washed out
    graded = ImageEnhance.Color(graded).enhance(1.35)
    orig = np.array(base, dtype=np.uint8)
...
def grade_noir(img: Image.Image, intensity: float = 1.0) -> Image.Image:
    """Cinematic noir: partial desaturation (~25% colour remaining) + amber warm cast
    + high contrast. Shadows crush to black; warm tones glow in highlights.
    Distinct from Dark Tones (fully BW+cool) — this retains a golden warmth.
    """
    base = img.convert("RGB")
    # Desaturate to ~25% colour — almost BW but not fully stripped
    desaturated = ImageEnhance.Color(base).enhance(0.25)
    arr = np.array(desaturated, dtype=np.float32)
    # Darken
    arr = (arr * 0.58).clip(0, 255)
    # Amber/golden warm cast: lift red strongly, lift green a touch, crush blue
    arr[:, :, 0] = (arr[:, :, 0] * 1.30).clip(0, 255)
    arr[:, :, 1] = (arr[:, :, 1] * 1.08).clip(0, 255)
    arr[:, :, 2] = (arr[:, :, 2] * 0.70).clip(0, 255)
    graded = Image.fromarray(arr.astype(np.uint8), "RGB")
    # High contrast: shadows go near-black, warm highlights pop
    graded = ImageEnhance.Contrast(graded).enhance(1.6)
    if intensity >= 0.999:
        return graded
    return Image.fromarray(blend(np.array(base), np.array(graded), intensity), "RGB")


def grade_midnight(img: Image.Image, intensity: float = 1.0) -> Image.Image:
